...
import pl.polsl.simon_go_manager.databinding.ItemGestureRecognizerResultBinding
...
class GestureRecognizerResultsAdapter :
    ...
    override fun onCreateViewHolder(
        ...
        val binding = FragmentGestureRecognitionBinding.inflate(
    ...
    inner class ViewHolder(private val binding: FragmentGestureRecognitionBinding) :
        RecyclerView.ViewHolder(binding.root) {

        fun bind(label: String?, score: Float?) {
            with(binding) {
                textLabel.text = label ?: NO_VALUE
                textScore.text = if (score != null) String.format(
                    Locale.US,
                    "%.2f",
                    score
                ) else NO_VALUE
